...
// Safe on reload + BFCache (Safari) + late session hydration
...
(() => {
  const boot = async () => {
    ...
    const safeStorage = (() => {
      ...
      try {
        ...
        try {
          ...
          return memory; // if this happens, auth cannot persist across reloads
        }
      }
    })();
    ...
    // --- Ensure a global UMD supabase exists (fallback inject if needed) ---
    async function ensureSupabaseGlobal() {
      if (window.supabase) return window.supabase;

      // Try to inject UMD build (more reliable for window.supabase)
      ...
      if (!window.supabase) throw new Error("Supabase global not available after load");
    ...
    let sb = null;
    try {
      const supa = await ensureSupabaseGlobal();
      ...
      if (
        ...
        throw new Error("Supabase URL/ANON key not configured properly.");
      }

      sb = supa.createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
        auth: {
          persistSession: true,
      ...
      console.error("[ArcadeHub] Supabase init failed:", e);
    }
    ...
    function showView(which) {
      ...
      if (views[which]) views[which].classList.add("active");
    ...
    async function ensureSession() {
      ...
      try {
        ...
        // refresh if token close to expiring
        if (sess?.expires_at) {
          const msLeft = sess.expires_at * 1000 - Date.now();
          if (msLeft < 60_000) {
        ...
        console.warn("[ArcadeHub] ensureSession failed:", e?.message || e);
        currentSession = null;
        return null;
      }
    }
    ...
    async function ensurePlayerProfile(session) {
      ...
      const { error: upErr } = await sb
        .from("players")
        .upsert({ user_id: user.id, username: fallback }, { onConflict: "user_id" });
    ...
    // ======================
    // 9) Wire UI (all actions ensureSession first)
    // ======================
    function wireUI() {
      // NAV
      ...
      if (btnContinue && playerNameInput) {
        btnContinue.addEventListener("click", async () => {
          ...
          const { error } = await sb
            .from("players")
            .upsert({ user_id: sess.user.id, username: nm }, { onConflict: "user_id" });
      ...
      // Logout (always resets UI)
      if (btnLogout) {
        btnLogout.addEventListener("click", async () => {
          if (!sb) return;
          try {
            await ensureSession();
            await sb.auth.signOut({ scope: "local" }); // most reliable
          } catch (e) {
    ...
    window.ArcadeHub = {
      ...
      // quick browser diag (no await needed)
      diag: () => ({
        hasSb: !!sb,
        storage:
    ...
    // ======================
    // 11) Boot + reload/tab-restore resilience
    // ======================
    wireUI();
    showView("home");
    setNameGate(false);
    ...
    // fixes Safari BFCache “reload breaks”
    ...
    if (sb) {
      sb.auth.onAuthStateChange(async (event, session) => {
        ...
        await refreshAuthUI(currentSession);
